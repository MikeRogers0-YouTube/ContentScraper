...
class V1::PageContentsTest < ActionDispatch::IntegrationTest
  test '.index - Show list of all indexed pages' do
    get v1_page_contents_path
    assert_response :success

    assert_match response.body, PageContent.all.to_json(only: [:url, :content]), 'Lists all the current PageContents'
  end
  ...
  test '.lookup - Will gracefully 404 when a URL is a redirect' do
    get lookup_v1_page_contents_path(url: 'http://www.mikerogers.io/')
    assert_response :not_found
  end
end
